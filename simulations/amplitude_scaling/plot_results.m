...
ap_unit =  8.5*mean(model.bootstrap_Est,2);
SIG0 = sum(ap_unit*mean(diff(model.freq)));
...
ax(1) = axes('Position',[0.18 0.15 0.23 0.23*8.5/6]);
    ...
    txt = text(0.75,0.3,'Model','fontsize',7,'color','k');
    txt = text(1.9,110,'Data','fontsize',7,'color',[0.5,0.5,0.5]);
    gcaformat;
ax(2) = axes('Position',[0.55 0.15 0.23 0.23*8.5/6]);
    dscale = [0,10.^linspace(-2,0,991),1.01:0.01:13];
